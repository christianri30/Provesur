		...
		bender.test( {
			...
			'test default CKEditor config': function() {
				bender.editorBot.create( {
					...
					assert.areEqual( bot.editor.config.exportPdf_fileName, 'export-pdf.pdf',
			...
			'test inaccessible stylesheets are handled correctly': function() {
				bender.editorBot.create( {
					name: 'editor8',
					config: exportPdfUtils.getDefaultConfig( 'unit', {
						contentsCss: 'https://cdn.jsdelivr.net/npm/bulma@0.9.0/css/bulma.min.css'
					} )
				}, function( bot ) {
					var editor = bot.editor,
						warnThrown = false,
					...
					editor.once( 'exportPdf', function( evt ) {
						...
						warnThrown ? assert.pass() : assert.fail();
					}, null, null, 19 );
